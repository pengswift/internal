...
func newDiskQueue(name string, dataPath string, maxBytesPerFile int64,
	...
	d := diskQueue{
		...
		Logger:            log.New(os.Stderr, fmt.Sprintf("[%s] ", name), log.Ldate|log.Ltime|log.Lmicroseconds),
	}
...
// while advacing read positions and rolling files, if necessary
// 读一数据
...
func (d *diskQueue) checkTailCorruption(depth int64) {
	...
	if depth != 0 {
		if depth < 0 {
			...
			d.logf(
				"ERROR: diskqueue(%s) positive depth at tail (%d), data loss, restting 0...",
				d.name, depth)
		}
		// force set depth 0
		atomic.StoreInt64(&d.depth, 0)
	...
	if d.readFileNum != d.writeFileNum || d.readPos != d.writePos {
		if d.readFileNum > d.writeFileNum {
			d.logf(
				"ERROR: diskqueue(%s) readFileNum > writeFileNum (%d > %d), corruption, skipping to next writeFileNum and restting 0...",
